...
def test_launcher_kernel_boot():
    ...
    try:
        ...
        logger.info(f"\n🤖 Registered agents: {', '.join(registry_keys)}")
...
def test_snapshot_has_agents():
    ...
    try:
        ...
        if not snapshot_path.exists():
            logger.warning(f"⚠️  Snapshot not found at {snapshot_path}")
        ...
        logger.info("\n✅ SNAPSHOT IS NOT EMPTY - All agents RUNNING!")
        return True
...
def main():
    ...
    if passed == total:
        logger.info("\n🎉 ALL TESTS PASSED! Launcher loads agents correctly.")
        return 0
    else:
