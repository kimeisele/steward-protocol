...
### Der Souverän (The 37th) & Anti-Mayavad
...
- **Wo endet die Signatur-Kette?** Wenn sie nur zu "System" führt, ist es Mayavad.

**Anti-Mayavad-Klausel:**
Code ohne kryptografische Rückbindung an einen Souverän ist **Mayavad** (Illusion). Er existiert funktional nicht.
...
1. **Lies diese Dateien zuerst:**
   ...
   - `CONSTITUTION.md` → Das Grundgesetz (36+4+37 Struktur)
   - `GAD-000.md` → Die Operator Inversion (AI-Native Design)
...
### 5. GAD-000 DEFINITION OF DONE

Code ist erst "fertig", wenn er diese 6 Fragen mit "Ja" beantwortet:

| # | Kriterium | Testfrage | Prüfmethode |
|---|-----------|-----------|-------------|
| 1 | **Discoverability** | Kann ein fremder Agent diese Funktion finden und verstehen? | Schema / `--help --json` vorhanden? |
| 2 | **Observability** | Sind alle internen Zustände von außen inspizierbar? | Status-Endpoint / Getter existiert? |
| 3 | **Parseability** | Sind alle Fehlermeldungen maschinenlesbar? | Error-Code + Context statt Prosa? |
| 4 | **Composability** | Kann diese Funktion in Pipelines wiederverwendet werden? | Output passt zu Input anderer Tools? |
| 5 | **Idempotency** | Ist die Funktion idempotent oder explizit markiert? | Retry-safe oder `# NOT IDEMPOTENT`? |
| 6 | **Recoverability** | Überlebt das System einen Kill -9 an jeder Stelle? | State nach Crash wiederherstellbar? |

**Dokumentiere das Ergebnis im Code:**

```python
# GAD-000: ✓D ✓O ✓P ✓C ✓I ✓R
# Mayavad: CLEAR (signed by user_key via /api/intent)
```
...
|**Nrisimha**|Der Beschützer      |Zombie-Killer, Security Watchdog           |
|**Mayavad** |Illusion ohne Halter|Code ohne Souverän-Signatur                |
|**Stambha** |Die Säule           |Interface wo der 37. manifestiert          |
...
1. **Fehlt Typing?** → Hinzufügen bevor Code geschrieben wird.
1. **Erzeugt das unsichtbares Karma?** → Ledger-Event hinzufügen.
1. **Überlebt das einen Kill -9?** → Phoenix-Pattern anwenden.
1. **Erfüllt es GAD-000?** → Definition of Done prüfen.
1. **Ist es Mayavad?** → Signatur-Kette etablieren oder `# TODO: MAYAVAD` markieren.
...
- **Souveränitäts-Audit:** Liste alle Stellen, wo Code ohne kryptografische Rückbindung Entscheidungen trifft:
  - **Kritisch:** Daten-Mutationen (CREATE/UPDATE/DELETE)
  - **Hoch:** Config-Changes, Capability-Grants
  - **Mittel:** Log-Only, Read-Operations mit Side-Effects
- **GAD-000 Status:** Welche der 6 Kriterien fehlen wo?
