Das ist der entscheidende Moment. Du hast absolut recht – wir müssen hier rigoros sein.

Ich sehe sofort das **kritische Problem** in deinem `setup.py`. Es ist der "Elefant im Raum".
Das `setup.py` behauptet, es sei `vibe-agency` (das OS). Das ist falsch. Das hier ist das **STEWARD PROTOCOL**. Wenn wir das so lassen, überschreiben wir bei der Installation das OS oder erzeugen Dependency Hölle.

Und um es "richtig fett" zu machen: Ein Protokoll ohne Kryptographie ist nur eine Textdatei. Wir rüsten jetzt **ECDSA Signaturen** nach. Damit ist ein Agent nicht nur "vorhanden", sondern **mathematisch beweisbar authentisch**.

Hier ist der **GOLDEN SHOT**. Wir fixen die Identität des Repos und implementieren den Trust-Layer in einem Aufwasch.

-----

### 🛠️ ARCHITECT INSTRUCTION TO BUILDER

**Objective:**

1.  **FIX Repo Identity:** Korrigiere `setup.py` von "vibe-agency" zu "steward-protocol".
2.  **IMPLEMENT Crypto Core:** Erstelle `steward/crypto.py` für asymmetrische Verschlüsselung (Keys generieren, Signieren, Verifizieren).
3.  **UPGRADE CLI:** Erweitere `steward` CLI um `keygen` und `sign` Befehle.

Führe diese Datei-Operationen exakt so aus:

#### 1\. `setup.py` – The Identity Fix

Wir definieren das Paket korrekt als Protokoll-Bibliothek und fügen `cryptography` als Dependency hinzu.
...
setup(
    name="steward-protocol",  # ✅ FIXED: Correct Identity
    version="0.1.0",
    ...
    install_requires=[
        "pydantic>=2.0.0",
        "pyyaml>=6.0",
        "cryptography>=41.0.0",  # ✅ NEW: Crypto Core
        "rich>=10.0.0",          # ✅ NEW: Beautiful CLI Output
    ],
    extras_require={
...
#### 2\. `steward/crypto.py` – The Trust Engine

Das Herzstück. Wir nutzen elliptische Kurven (SECP256R1), da dies der Industriestandard für WebAuthn und moderne Krypto ist.
...
#### 3\. `steward/cli.py` – The Implementation
...
1.  Führe den Code oben aus.
2.  Installiere neu: `pip install -e .`
3.  Generiere Identity: `steward keygen`
4.  Signiere Root: `steward sign STEWARD.md --append`

Damit ist das Repo sauber, die Identität korrekt (`steward-protocol`), und wir sind ready für echten, kryptographisch gesicherten Agenten-Traffic.
