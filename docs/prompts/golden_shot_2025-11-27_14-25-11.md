...
Gap 3.2: Rename StewardAgent → DiscovererAgent (57 references)
...
**Problem:** 57 references to "StewardAgent" across codebase
...
# 2. Update class name
sed -i 's/class StewardAgent/class DiscovererAgent/g' steward/system_agents/discoverer/agent.py

# 3. Update all 57 references
grep -r "StewardAgent" --include="*.py" --include="*.md" -l | xargs sed -i 's/StewardAgent/DiscovererAgent/g'
...
✅ 0 references to "StewardAgent" remaining
✅ All tests pass
...
# After StewardAgent rename
git add [all changed files]
git commit -m "refactor: Rename StewardAgent → DiscovererAgent - P1 Gap 3.2 Closure

BEFORE: StewardAgent (57 references, naming confusion)
AFTER: DiscovererAgent (clear naming, GAD-000 compliant)

Changes:
- Renamed class: StewardAgent → DiscovererAgent
