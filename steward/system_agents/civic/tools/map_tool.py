...
## 🕉️ BHU-MANDALA TOPOLOGY (Sacred Geometry)

Agent City is not a flat hierarchy. It is a **MANDALA** — a sacred, concentric structure
...
"""
    ...
    def write_citymap(self, output_file: str = "CITYMAP.md") -> bool:
        ...
        try:
            ...
            print(f"✅ City Map generated: {output_path}")
            print(f"📍 Mapped {len(self.agents)} agents")
            print(f"🔗 Found {sum(len(d) for d in self.dependencies.values())} dependencies")

            return True

        except Exception as e:
            print(f"❌ Error writing city map: {e}")
            return False
