...
class DashboardRenderer(Tool):
    ...
    def _scan_and_render(self) -> str:
        """Scan system and render DASHBOARD.md."""
        # Gather all data
        ...
        content = f"""# AGENT CITY DASHBOARD
...
## {health_icon} SYSTEM HEALTH: {health_text}

```
+------------------------------------------------------------------+
|                    AGENT CITY STATUS                              |
+------------------------------------------------------------------+
|  System:    {health["status"]:<10}                                         |
|  Agents:    {agent_counts["total"]:>3} total ({agent_counts["system"]} System + {agent_counts["citizen"]} Citizens)             |
|  Kernel:    {"ACTIVE" if (self.root_dir / "vibe_core").exists() else "MISSING":<10}                                         |
|  Ledger:    {"INITIALIZED" if (self.root_dir / ".steward" / "ledger.json").exists() else "NOT READY":<12}                                       |
+------------------------------------------------------------------+
```

{self._render_issues(health["issues"])}
...
{self._render_git_status(git_activity)}
