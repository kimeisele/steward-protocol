...
from vibe_core.mahamantra.substrate.algorithm.maha import (
    MahaModularSynth, SYNTH_PRESETS, MahaSynthParams,
)
...
def fingerprint_distance(fp_a: Tuple[Tuple[int, ...], ...], 
                          fp_b: Tuple[Tuple[int, ...], ...]) -> float:
    ...
    for preset_a, preset_b in zip(fp_a, fp_b):
        ...
        dist = sum(abs(hist_a.get(k, 0)/total_a - hist_b.get(k, 0)/total_b) 
                    for k in all_keys)
...
WORDS = ["the", "and", "not", "but", "came", "preach", "gospel", "krishna", 
         "consciousness", "boys", "girls", "some", "exactly"]
