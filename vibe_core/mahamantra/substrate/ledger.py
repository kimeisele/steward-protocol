...
class SQLiteLedger(VibeLedger):
    ...
    def verify_chain_integrity(self) -> Dict[str, object]:
        ...
        if corruptions:
            ...
            return {
                "status": "CORRUPTED",
                "message": f"Ledger integrity check failed: {len(corruptions)} violations found",
                "total_events": len(events),
                "corrupted": True,
                "corruptions": corruptions,
            }

        logger.info(f"✅ Ledger integrity verified ({len(events)} events)")
        return {
            "status": "CLEAN",
            "message": "Ledger integrity check passed",
            "total_events": len(events),
            "corrupted": False,
        }
