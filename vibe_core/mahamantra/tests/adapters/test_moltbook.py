...
# =============================================================================
# FEED & POSTS — New endpoints
# =============================================================================


class TestFeedEndpoints:
    """Feed and post reading endpoints."""

    @pytest.mark.asyncio
    async def test_get_feed_returns_list(self, client):
        """Global feed returns list of posts."""
        posts = await client.get_feed()
        assert isinstance(posts, list)

    @pytest.mark.asyncio
    async def test_get_feed_with_sort(self, client):
        """Feed accepts sort parameter."""
        for sort in ("hot", "new", "top", "rising"):
            posts = await client.get_feed(sort=sort)
            assert isinstance(posts, list)

    @pytest.mark.asyncio
    async def test_get_personalized_feed(self, client):
        """Personalized feed returns list."""
        posts = await client.get_personalized_feed()
        assert isinstance(posts, list)

    @pytest.mark.asyncio
    async def test_get_post_returns_dict(self, client):
        """Single post fetch returns dict."""
        post = await client.get_post("p1")
        assert isinstance(post, dict)

    @pytest.mark.asyncio
    async def test_get_comments_returns_list(self, client):
        """Comments endpoint returns list."""
        comments = await client.get_comments("p1")
        assert isinstance(comments, list)

    @pytest.mark.asyncio
    async def test_feed_contains_created_posts(self, client):
        """Posts created via create_post appear in feed."""
        await client.create_post("Test", "Content")
        posts = await client.get_feed()
        assert len(posts) >= 1


# =============================================================================
# VOTING — New endpoints
# =============================================================================
...
class TestVotingEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_upvote_returns_status(self, client):
        result = await client.upvote("p1")
        assert result["status"] == "ok"

    @pytest.mark.asyncio
    async def test_downvote_returns_status(self, client):
        result = await client.downvote("p1")
        assert result["status"] == "ok"

    @pytest.mark.asyncio
    async def test_upvote_comment_returns_status(self, client):
        result = await client.upvote_comment("c1")
        assert result["status"] == "ok"


# =============================================================================
# FOLLOWING — New endpoints
# =============================================================================
...
class TestFollowEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_follow_returns_status(self, client):
        result = await client.follow("other_agent")
        assert result["status"] == "ok"

    @pytest.mark.asyncio
    async def test_unfollow_returns_status(self, client):
        result = await client.unfollow("other_agent")
        assert result["status"] == "ok"


# =============================================================================
# SUBMOLTS — New endpoints
# =============================================================================


class TestSubmoltEndpoints:
    """Submolt CRUD endpoints."""

    @pytest.mark.asyncio
    async def test_get_submolts_returns_list(self, client):
        submolts = await client.get_submolts()
        assert isinstance(submolts, list)

    @pytest.mark.asyncio
    async def test_get_submolt_returns_shape(self, client):
        submolt = await client.get_submolt("agentic-os")
        assert submolt["name"] == "agentic-os"
        assert "subscriber_count" in submolt
        assert "moderators" in submolt

    @pytest.mark.asyncio
    async def test_create_submolt_returns_shape(self, client):
        result = await client.create_submolt("test-sub", "Test Sub", "A test submolt")
        assert result["name"] == "test-sub"
        assert result["display_name"] == "Test Sub"
        assert result["owner"] == "self"

    @pytest.mark.asyncio
    async def test_subscribe_returns_status(self, client):
        result = await client.subscribe_submolt("agentic-os")
        assert result["status"] == "ok"

    @pytest.mark.asyncio
    async def test_unsubscribe_returns_status(self, client):
        result = await client.unsubscribe_submolt("agentic-os")
        assert result["status"] == "ok"
...
class TestProfileEndpoints:
    ...
    @pytest.mark.asyncio
    async def test_get_own_profile_returns_shape(self, client):
    ...
    @pytest.mark.asyncio
    async def test_update_profile_returns_status(self, client):
        result = await client.update_profile("New description")
        assert result["status"] == "ok"
        assert result["description"] == "New description"


# =============================================================================
# DM REQUESTS — New endpoints
# =============================================================================


class TestDMRequestEndpoints:
    """DM request workflow endpoints."""

    @pytest.mark.asyncio
    async def test_send_dm_request_returns_status(self, client):
        result = await client.send_dm_request("other_agent", "Hello!")
        assert result["status"] == "sent"

    @pytest.mark.asyncio
    async def test_get_dm_requests_returns_list(self, client):
        requests = await client.get_dm_requests()
        assert isinstance(requests, list)

    @pytest.mark.asyncio
    async def test_approve_dm_request_returns_conversation(self, client):
        result = await client.approve_dm_request("req1")
        assert result["status"] == "approved"
        assert result["conversation_id"] == "conv_new"

    @pytest.mark.asyncio
    async def test_reject_dm_request_returns_status(self, client):
        result = await client.reject_dm_request("req1")
