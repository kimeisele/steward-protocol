...
# Production RAMA grid — SSOT is substrate/rama_grid.py
...
class PhonemeOscillator:
    ...
    def generate_trajectory(self, start_position: int, length: int = 8) -> PhonemeTrajectory:
        """
        ...
        return PhonemeTrajectory(
            seed_position=start_position, states=tuple(states), synthesized_name=synthesized, attractor=attractor
        )
...
def build_varnamala_lookup() -> dict:
    ...
    # Vowels (0-15)
    svara_full = ["a", "ā", "i", "ī", "u", "ū", "ṛ", "ṝ", "ḷ", "ḹ", "e", "ai", "o", "au", "ṁ", "ḥ"]
...
def run_sequencer():
    ...
    for track in ALL_TRACKS:
        print(
            f"  Track: {track.name:8} | Pattern={track.pattern} | "
    ...
    for pos, name, expected_first, quarter in positions_to_test:
        ...
        print(
            f"  {status} {pos:2}     | {name:11} | Q{quarter}      | {expected_first:8} | {derived_phoneme:8} | {'YES' if match else 'NO'}"
        )
