...
def analyze_derivation_pattern() -> dict:
    ...
    if len(set(deltas)) == 1:
        analysis["potential_formulas"].append(
            f"position = (vibration + {deltas[0]}) % WORDS"
        )
    else:
...
def run_analysis():
    ...
    for name, data in mahajana_results["vibrations"].items():
        print(f"  {name:12} | vib={data['vibration']:5} | pos={data['computed_pos']:2} | "
    ...
    if analysis["potential_formulas"]:
        ...
        print("\n  No consistent formula found - deeper analysis needed")
    ...
    print("=" * 70)
    print("""
Die Vibration-basierte Ableitung ist PARTIELL korrekt.

NÄCHSTE SCHRITTE:
1. Analysiere ob es einen OFFSET gibt der die Positionen alignt
2. Prüfe ob die Reihenfolge in SB 6.3.20 selbst eine Mathe hat
3. Untersuche ob guna/varna einen adjustment factor definiert

Die INFRASTRUKTUR (VibrationSignature) ist da.
Die MATHEMATIK (mod-16, position sums) ist da.
Es fehlt nur die richtige KOMBINATION.
    """)
...
def analyze_verse_order():
    ...
    sb_order = [
        ("brahma", 1), ("narada", 2), ("shambhu", 3), ("kumaras", 4),
        ("kapila", 5), ("manu", 6), ("prahlada", 7), ("janaka", 8),
        ("bhishma", 9), ("bali", 10), ("shuka", 11), ("yamaraja", 12)
    ]
...
def quarter_based_derivation():
    ...
    for profile in MAHAJANA_PROFILES:
        ...
        print(f"  {match} {profile.name:12} | Q{profile.quarter} | mod_{quarter_mod:2}={step1:3} | "
...
def analyze_mahajana_names_as_seed():
    ...
    print("  " + "-" * 56)
    for d in data:
        print(f"  {d['name']:12} | {d['vib']:9} | {d['expected']:3} | {d['vib_mod_49']:4} | "
