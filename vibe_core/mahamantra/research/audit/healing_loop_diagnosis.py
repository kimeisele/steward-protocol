...
def check_shuddhi_subscriber_dryrun() -> LoopCheck:
    ...
    try:
        ...
        if sub._dry_run:
            return LoopCheck(
                "shuddhi_subscriber.dry_run", "OPEN",
...
def check_kg_has_violations() -> LoopCheck:
    ...
    try:
        ...
        return LoopCheck(
            "KG.violations", "CLOSED" if violations else "OPEN",
            f"{len(violations)} unhealed violations in KG"
        )
    except Exception as e:
...
def check_engine_remedies() -> LoopCheck:
    ...
    try:
        ...
        return LoopCheck(
            "engine.remedies", "CLOSED" if healers else "OPEN",
            f"{len(remedies)} total: {len(healers)} healers {healers}, "
            f"{len(detectors)} detect-only {detectors}"
        )
    except Exception as e:
...
def check_venu_heartbeat() -> LoopCheck:
    ...
    try:
        ...
        if not names:
            return LoopCheck(
                "venu.subscribers", "OPEN",
                "No BeatSubscribers discovered. The heartbeat has no listeners."
            )
        return LoopCheck(
            "venu.subscribers", "CLOSED",
            f"{len(names)} subscribers: {names}"
        )
    except Exception as e:
