...
ONE IMPORT:
    from vibe_core.protocols.mahajanas.<whoever-sits-at-4>.types.lineage import LineageChain, LineageBlock

Or via this bridge (for backward compatibility):
    from vibe_core.lineage import LineageChain, LineageBlock
...
def __getattr__(name: str) -> Any:
    ...
    attr = MahajanaRegistry.load_type(position=4, type_name=name)

    if attr is not None:
        return attr

    # Fallback: Try loading entire types module and get any attribute
    types_module = MahajanaRegistry.load_module(index=4, component="types")
