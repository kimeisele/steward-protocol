        ...
        for edge in graph.edges:
            if edge.from_node not in adjacency or edge.to_node not in adjacency:
                return ExecutionPlan(
                    ...
                    dependencies={},
                    is_valid=False,
                    errors=[f"Edge references non-existent node: {edge.from_node} → {edge.to_node}"],
                )
    ...
    def execute_step(self, graph: WorkflowGraph, node_id: str, context: str | None = None) -> ExecutionResult:
        ...
        if node.prompt_key:
            try:
                ...
                logger.info(f"🎙️ VOICE: Retrieved prompt from registry: {node.prompt_key} ({len(base_prompt)} chars)")
            except Exception as e:
                logger.warning(
        ...
        if node.knowledge_context:
            try:
                ...
                if hits:
                    ...
                    knowledge_section += (
                        "The following knowledge artifacts from our knowledge base are relevant to this task:\n\n"
                    )
