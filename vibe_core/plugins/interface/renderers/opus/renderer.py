...
Configuration lives in config/opus.yaml - NO HARDCODING.
"""
...
class OpusRenderer(BaseRenderer):
    ...
    def _render_header(self) -> str:
        """Render document header with philosophy from config."""
        timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
        status = getattr(self.kernel, "status", "UNKNOWN")

        # Load philosophy from config
...
```
{philosophy.strip() if philosophy else "Load config/opus.yaml for philosophy"}
```
...
```
{harness_task.strip() if harness_task else "Load config/opus.yaml for harness_task"}
```
