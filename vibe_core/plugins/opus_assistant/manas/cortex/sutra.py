    ...
    for page_payload in payload.get("pages", []):
        specs.append(
            WikiSurfacePageSpec(
                ...
                authority=str(page_payload.get("authority") or "derived"),
                domain=str(page_payload.get("domain") or page_payload.get("section") or "reference"),
                ...
                query_aliases=tuple(str(value).lower() for value in page_payload.get("query_aliases", [])),
                render_config=dict(page_payload.get("render_config") or {}),
            )
        )
