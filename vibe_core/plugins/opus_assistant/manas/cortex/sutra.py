...
Wiki Pages Generated:
    ...
    - Start-Here.md: Canonical README binding
    - Constitution.md: Canonical constitutional source
    - Governance-Index.md: Derived governance navigation
    - AGI-Manifesto.md: Canonical doctrine binding
    - Protocols.md: Canonical protocol registry binding
    - Architecture.md: Canonical architecture binding
...
TEMPLATE_GOVERNANCE_INDEX = """# Governance Index
...
def _normalize_bound_markdown(content: str) -> str:
    """Apply minimal formatting fixes when binding authored markdown into wiki pages."""
...
def _load_surface_model(workspace: Path) -> tuple[list[dict[str, str]], list[WikiSurfacePageSpec]]:
    ...
    for page_payload in payload.get("pages", []):
        specs.append(
            WikiSurfacePageSpec(
                page_type=WikiPageType[page_payload["id"]],
                title=str(page_payload.get("title") or page_payload["id"]),
                wiki_name=str(page_payload.get("wiki_name") or WikiPageType[page_payload["id"]].value),
                page_class=str(page_payload.get("page_class") or "derived"),
                section=str(page_payload.get("section") or "reference"),
                nav_label=str(page_payload.get("nav_label") or page_payload.get("title") or page_payload["id"]),
                description=str(page_payload.get("description") or ""),
                renderer=str(page_payload.get("renderer") or ""),
                source_path=page_payload.get("source_path"),
                featured=bool(page_payload.get("featured", False)),
                include_in_sidebar=bool(page_payload.get("include_in_sidebar", True)),
            )
        )
...
class SutraWeaver:
    ...
    def _ordered_surface_specs(self) -> list[WikiSurfacePageSpec]:
        ...
        return list(self._surface_specs.values())
    ...
    def _build_home_quick_links(self) -> str:
        ...
        for spec in self._ordered_surface_specs():
            ...
            lines.append(f"- [[{spec.wiki_name}|{label}]]{description}")
        ...
        for section in self._surface_sections:
            ...
            for spec in section_pages:
                ...
                if label == spec.wiki_name:
                    lines.append(f"- [[{spec.wiki_name}]]")
                else:
                    lines.append(f"- [[{spec.wiki_name}|{label}]]")
            lines.append("")
        return "\n".join(lines).strip()
    ...
    def _weave_canonical_page(self, spec: WikiSurfacePageSpec) -> WikiPage:
        ...
        if source_path.exists():
            content = _normalize_bound_markdown(source_path.read_text())
        else:
            content = f"# {spec.title}\n\n_Source document not found: `{spec.source_path}`_\n"
        content = content.rstrip() + f"\n\n---\n\n*Canonical source: `{spec.source_path}`*\n"
        return WikiPage(
            page_type=spec.page_type,
            title=spec.title,
            content=content,
    ...
    def _weave_governance_index(self, spec: WikiSurfacePageSpec) -> WikiPage:
        ...
        if governance_dir.exists():
            for path in sorted(governance_dir.iterdir()):
                ...
                governance_sources.append(f"- **{label}** — `{path.relative_to(self._workspace).as_posix()}`")
        governance_section = "\n".join(governance_sources) or "_Governance sources not loaded_"

        canonical_links = [
            "- [[Constitution]] - Supreme law and root of trust",
            "- [[Protocols]] - Living protocol registry",
            "- [[AGI-Manifesto|AGI Manifesto]] - Governed intelligence doctrine",
        ]
        ...
        return WikiPage(
            ...
            title=spec.title,
            content=content,
            source_entities=["CONSTITUTION.md"] + [str(path.split("`")[1]) for path in governance_sources],
        )
...
def handle_sutra_query(content: str, workspace: Optional[Path] = None) -> str:
    ...
    if any(word in content_lower for word in ["preview", "show", "display"]):
        ...
        elif "manifesto" in content_lower:
            page_type = WikiPageType.AGI_MANIFESTO
        elif "protocol" in content_lower:
            page_type = WikiPageType.PROTOCOLS
        elif "architecture" in content_lower:
            page_type = WikiPageType.ARCHITECTURE
        elif "start" in content_lower or "readme" in content_lower:
            page_type = WikiPageType.START_HERE
        elif "law" in content_lower or "constitution" in content_lower:
            page_type = WikiPageType.CONSTITUTION
        elif "governance" in content_lower:
            page_type = WikiPageType.GOVERNANCE_INDEX
        elif "map" in content_lower:
...
- "preview governance" - Show governance index preview
...
- Start-Here.md - Canonical README binding
- Constitution.md - Canonical constitution page
- Governance-Index.md - Governance navigation
