...
class SankalpaOrchestrator:
    ...
    async def inject_intent(self, intent: SankalpaIntent) -> Dict[str, Any]:
        """
        Inject a SankalpaIntent into ChatService via LotusTask.

        ANTARYAMI PATH: Sankalpa → LotusTask → ChatService.inject_task()

        This is the PROACTIVE VOICE - tasks that arise from within,
        not from user input. The system acts on its own will (Sankalpa).

        FLOW:
        1. Wrap intent in LotusTask (meditation container)
        2. Inject into ChatService (the execution brain)
        3. ChatService runs it through Lotus lifecycle
        4. Return result with Dhyana state (fold depth, japa progress)

        Args:
            intent: SankalpaIntent to execute

        Returns:
            Dict with execution result + meditation state
        """
        # Create LotusTask from intent
        lotus_task = self._planner.create_lotus_task(intent)

        logger.info(
            f"🔥 SANKALPA INJECT: {intent.id} ({intent.intent_type}) → ChatService"
        )

        try:
            # Get ChatService and inject
            from vibe_core.services.chat_service import get_chat_service

            chat_service = get_chat_service()
            result = await chat_service.inject_task(lotus_task)

            logger.info(
                f"✅ SANKALPA COMPLETE: {intent.id} → {result.get('dhyana_state', 'unknown')}"
            )
            ...
            return {
                ...
                "dhyana_state": result.get("dhyana_state"),
                "fold_depth": result.get("fold_depth"),
                "japa_progress": result.get("japa_progress"),
                "mahajana": result.get("mahajana"),
                "execution": result.get("execution"),
            }

        except Exception as e:
            logger.error(f"❌ SANKALPA INJECT failed: {e}")
            return {
                "success": False,
                "intent_id": intent.id,
                "error": str(e),
    ...
    async def execute_pending(
        ...
        """
        Evaluate strategies, generate intents, and inject them into ChatService.

        FULL SANKALPA CYCLE:
        1. Evaluate all strategies (check triggers, constraints)
        2. Generate intents for those that should fire
        3. Inject each intent into ChatService via LotusTask
        4. Return results with meditation states

        This is the PROACTIVE HEARTBEAT of the system.
        ...
        """
        # Generate intents from strategies
        intents = self.think(
            context=context,
            idle_minutes=idle_minutes,
            pending_intents=pending_intents,
        ...
        for intent in intents:
            result = await self.inject_intent(intent)
            results.append(result)

        logger.info(
            f"🔥 SANKALPA CYCLE: {len(results)} intents executed, "
