...
from vibe_core.protocols.universal.sudarshana import KERNEL, mantra_governed
...
def test_sudarshana_autodiscovery(mock_kernel_pool):
    ...
    ctx = SovereignContext(
        identity_id="did:test:user", signature="sig", tattva_level=TranscendentalQuality.TRUTHFULNESS
    )
...
def test_sudarshana_immune_response(mock_kernel_pool):
    ...
    failure_payload = PranaFailure(error=simulated_error, traceback="Mock Traceback", task_id="test_crash_id")
