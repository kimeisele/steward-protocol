...
def test_boot_protocol_ignite_requires_genesis():
    ...
    assert "genesis" in sig.parameters, "MAYAVAD: BootProtocol.ignite() allows anonymous ignition!"
...
def test_genesis_byte_logic():
    ...
    g_valid = GenesisByte(
        signature="valid_sig_123", resonance=MantraByte.standard_16(), dimension=16, timestamp=123456789.0
    )
...
def test_legacy_boot_violation():
    ...
    if legacy_violations:
        pytest.skip(f"EXPECTED LEGACY VIOLATION (RED state): {legacy_violations}. ")
