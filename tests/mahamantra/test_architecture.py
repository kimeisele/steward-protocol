...
from vibe_core.mahamantra.protocols import _srivasa as srivasa_proto
...
def test_pancha_tattva_integrity():
    """Verify that all 5 Pancha Tattva protocols exist and are importable."""
    assert seed_proto, "Ishvara (Seed) protocol missing"
    assert pancha_proto, "Identity (Pancha) protocol missing"
    assert srivasa_proto, "Governance (Srivasa) protocol missing"
...
def test_protocol_definitions():
    ...
    assert "__tattva__" in dir(pancha_proto.PanchaTattvaProtocol), \
        "PanchaTattvaProtocol missing __tattva__ property"
        
    # KalaProtocol must define tick()
    assert "tick" in dir(kala_proto.KalaProtocol), \
           "KalaProtocol missing tick() method"

    # KarmaProtocol must define execute()
    assert "execute" in dir(karma_proto.KarmaProtocol), \
           "KarmaProtocol missing execute() method"
