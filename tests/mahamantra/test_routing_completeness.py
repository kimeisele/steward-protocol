...
class TestMahamantraRouting:
    ...
    def test_all_subpackages_route(self, mahamantra, mahamantra_root):
        ...
        subpackages = [
            d.name
            for d in mahamantra_root.iterdir()
    ...
    def test_all_root_modules_route(self, mahamantra, mahamantra_root):
        ...
        root_modules = [
            f.stem for f in mahamantra_root.glob("*.py") if f.stem != "__init__" and not f.stem.startswith("_")
        ]
    ...
    def test_all_adapters_route(self, mahamantra, mahamantra_root):
        ...
        adapters = [f.stem for f in adapters_dir.glob("*.py") if f.stem != "__init__" and not f.stem.startswith("_")]
...
class TestMahamantraRoutingCount:
    ...
    def test_minimum_subpackages(self):
        ...
        subpackages = [
            d.name for d in root.iterdir() if d.is_dir() and (d / "__init__.py").exists() and not d.name.startswith("_")
        ]
...
class TestResearchFractalRouting:
    ...
    @pytest.mark.xfail(
        reason="research moved to mahamantra_research/ — fractal routing via mahamantra.research no longer applies"
    )
    def test_all_research_modules_route(self, mahamantra, research_root):
        ...
        research_modules = [
            f.stem for f in research_root.glob("*.py") if f.stem != "__init__" and not f.stem.startswith("_")
        ]
...
class TestAdaptersFractalRouting:
    ...
    def test_all_adapter_modules_route(self, mahamantra, adapters_root):
        ...
        adapter_modules = [
            f.stem for f in adapters_root.glob("*.py") if f.stem != "__init__" and not f.stem.startswith("_")
        ]
...
class TestSubstrateFractalRouting:
    ...
    def test_all_substrate_modules_route(self, mahamantra, substrate_root):
        ...
        substrate_modules = [
            f.stem for f in substrate_root.glob("*.py") if f.stem != "__init__" and not f.stem.startswith("_")
        ]
...
class TestProtocolsFractalRouting:
    ...
    def test_protocols_subpackages_route(self, mahamantra, protocols_root):
        ...
        subpackages = [
            d.name
            for d in protocols_root.iterdir()
            if d.is_dir() and (d / "__init__.py").exists() and not d.name.startswith("_")
        ]
