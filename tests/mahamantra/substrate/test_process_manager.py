...
def test_spawn_and_communication(process_manager):
    ...
    process_manager.spawn_agent(
        agent_id=agent_id, cartridge_path=FIXTURE_PATH, cartridge_class_name="DummyAgent", config={"deployment": "test"}
    )
...
def test_narasimha_crash_recovery(process_manager):
    ...
    # 1. Spawn
    process_manager.spawn_agent(agent_id=agent_id, cartridge_path=FIXTURE_PATH, cartridge_class_name="DummyAgent")
    time.sleep(1)
