...
class TestKaliyaStream:
    ...
    async def test_kaliya_flood_attack(self):
        ...
        assert bus_status['history_size'] <= 1000, \
            f"History overflow! Size={bus_status['history_size']}, max=1000"
        print("   ✅ History limit respected (no memory explosion)")
        
        # 7. Verify bus is still responsive
        test_event = Event(
            event_type="KRISHNA_DANCE",
            agent_id="test",
            message="Can the bus still work?"
        )
        ...
        assert result.events_per_second >= 100, \
            f"Too slow: {result.events_per_second} events/sec"
    ...
    async def test_kaliya_ouroboros_detection(self):
        ...
        for _ in range(50):
            event = EventClass(
                ...
                agent_id="kaliya",
                details={"causation_id": my_id, "reply_to": my_id}
            )
            await bus.emit(event)
